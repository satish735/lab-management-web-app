...
const ViewSlots = ({ loading = false, slots = [], editable = false, setSlots = () => { } }) => {
            ...
            {!loading && slots.length > 0 && <div className='w-100'>
                {slots.map((slotItem, index) => {
                    ...
                    return <><div onClick={() => setSelectedSlot(isSlotSelected ? null : slotItem)} className={` slot-date-section ${color_class} ${isSlotSelected ? "selected" : ""}`} key={index}>
                        ...
                        {editable == true && <FaXmark className='remove-icon' onClick={() => { removeSlot(slotItem?.date) }} />}
                                    ...
                                    <div key={category}>
                                        ...
                                        {slots.map((slot, index) => {
                                            ...
                                            return <div key={index} className='slot-time-block'>
                                                ...
                                                {editable == true && <FaXmark className='remove-icon' onClick={() => { removeSlotTime(slotItem?.date, slot?.slotStartTime) }} />}
                                            </div>
                                        }
                                        )}
                                    </div>
