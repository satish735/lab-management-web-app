...
const UpcomingSlots = ({ selectedSlot = null, onChange = () => { }, setslotdata = () => { } }) => {
  ...
  const [getUpcomingSlotsResponse, getUpcomingSlotsHandler] = useAPI(
    ...
    (e) => {
      toast.error(
        transformErrorDefault("Something went wrong while Fetching Upcoming Slots!", e)
      );
      return e;
    }
  );
  const groupedSlots = groupTimeSlots((selectedSlotDate?.slotTimes ?? []).filter(slotTimeItem => slotTimeItem?.type != "removed"));
  return (
    <div className="w-100 text-center">
      <div className="upcoming-slots-date-section">
        {getUpcomingSlotsResponse?.fetching && <div className="loading">
          <Spinner
            style={{
              height: "3rem",
              width: "3rem",
        ...
        {(!getUpcomingSlotsResponse?.data || getUpcomingSlotsResponse?.data?.length == 0) && !getUpcomingSlotsResponse?.fetching && (
        ...
        {!getUpcomingSlotsResponse?.fetching && Array.isArray(getUpcomingSlotsResponse?.data) &&
          <div className='w-100 text-start'>
            {(getUpcomingSlotsResponse?.data ?? []).filter(slotItem => slotItem?.type != "removed").map((slotItem, index) => {
              var color_class = slotItem?.status == "disabled" ?
                ...
                moment(slotItem?.date).isBefore(moment(), 'day') ?
                  ...
                  moment(slotItem?.date).isSame(moment(), 'day') ?
                    "current" :
                    "upcoming"
              var isSlotSelected = selectedSlotDate?.date && moment(slotItem?.date).isSame(moment(selectedSlotDate?.date), 'day') ? true : false
              return <div onClick={() => setSelectedSlotDate(isSlotSelected ? null : slotItem)} className={` slot-date-section ${color_class} ${isSlotSelected ? "selected" : ""}`} key={index}>
      ...
      {selectedSlotDate && <div className="upcoming-slots-time-section">
        {Object.entries(groupedSlots).map(([category, slots]) => (
          slots.length > 0 && (
            <div key={category}>
              <h2>{category} - Timings</h2>
              {slots.map((slot, index) => {
                var slot_color_class = slot?.status == "active" ? (
                  slot?.maxUse == slot?.currentUse ? "full" : (
                    moment(`${selectedSlotDate?.date} ${slot?.slotStartTime}`, 'YYYY-MM-DD hh:mm A').isBefore(moment()) ? "disabled" : "upcoming"
                  )
                ) :
                  "disabled"
                return <div key={index} className={`slot-time-block ${slot_color_class} ${selectedSlot == slot?._id ? "selected" : ""
                  }`}
                  onClick={() => {
                    if (slot_color_class == "full") {
                      toast.error("Slot is fully booked.Please select available slot Time!")
                    }
                    else if (slot_color_class == "past" || slot_color_class == "disabled") {
