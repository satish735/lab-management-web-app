...
const CreateDiscountCoupen = () => {
    ...
    const [DiscountCoupenResponse, DiscountCoupenHandler] = useAPI(
        ...
        (e) => {
            ...
            CouponName.setEnteredValue()
            CouponCode.setEnteredValue()
            ExpirationDate.setEnteredValue()
            StartDate.setEnteredValue()
            MaxUse.setEnteredValue()
            DescriptionShort.setEnteredValue()
            maxUsagePerUser.setEnteredValue()
            DiscountValue.setEnteredValue()
            setStatus()
            setselectUserType()
            setSelectCenter([])
            setSelectTests([])
        ...
        (e) => {
            ...
            toast.error(
                transformErrorDefault(
                    "Something went wrong while adding Coupen!",
                    e
                )
            );
            return e;
    ...
    const submit = () => {
        ...
        console.log({
            ...
            criteriaFields: (selectedCriteria ? (selectedCriteria ?? []).map((item) => item.value) : []),
        });
        ...
        if (!CouponNameValidate ||
            ...
            DiscountCoupenHandler({
                body: {
                    ...
                    discountType: DiscountType ?? '',
                    selectedCriteria: FieldsToDisplay?.all == true ?
                        {
                            testCondition: [],
                    ...
                    <div className="row">
                        ...
                        <div className="my-3">
                            ...
                            {(TermsCondition ?? []).map((positionItem, index) => {
                                return <ExperienceBlog positionItem={positionItem} key={index} setTermsCondition={setTermsCondition} length={(TermsCondition ?? []).length} />
                            })}
...
export default CreateDiscountCoupen;





const ExperienceBlog = ({ positionItem, key, setTermsCondition, length }) => {
