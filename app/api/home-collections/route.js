  ...
  try {
    ...
    const searchFilter = searchQuery
      ? {
          $or: [
            {
              bookingId: { bookingId: { $regex: searchQuery, $options: "i" } },
            },
            { collectedByName: { $regex: searchQuery, $options: "i" } },
            { collectionStatus: { $regex: searchQuery, $options: "i" } },
          ],
        }
      : {};
    if (centerId) {
    //   searchFilter.bookingId = { centerId: centerId };
    }
    ...
    // Return response
    return new Response(
      JSON.stringify({ data: homeCollectionList, total: totalCount }),
      { status: 200 }
    );
  } catch (error) {
...
export const PUT = async (request, { params }) => {
  try {
    ...
    if (!homeCollectionId || !type) {
      return new Response("Home Collecation and type cannot be null!", {
        status: 400,
      });
    }
    ...
    if (!homeCollection) {
      return new Response("No Home Collection found with given id!", {
        status: 404,
      });
    }
    switch (type) {
      case "confirmed":
        ...
        var newActivity = new HomeCollectionActivity({
          ...
          description:
            ...
            "<span>HomeCollection status has been changed to <span class='booking-badge confirmed'>Confirmed</span>.<span>",
        });
      ...
      case "picked":
        ...
        homeCollection.samplesCollected = Array.isArray(
          restBody?.samples_collected
        )
          ? restBody?.samples_collected
          : [];
        await homeCollection.save();
        var newActivity = new HomeCollectionActivity({
          ...
          description:
            restBody?.description ??
            "<span>HomeCollection status has been changed to <span class='booking-badge picked'>Picked</span>.<span>",
        });
        await newActivity.save();
        break;
      case "cancelled":
        ...
        var newActivity = new HomeCollectionActivity({
          ...
          description:
            restBody?.description ??
            "<span>HomeCollection status has been changed to <span class='booking-badge cancelled'>Cancelled</span>.<span>",
        });
        await newActivity.save();
        break;
      default:
    ...
    return new Response(
      { message: "Home Collection updated Successfully.", type },
      { status: 200 }
    );
  } catch (error) {
    console.log(error);
...
export const POST = async (request, { params }) => {
  try {
    ...
    if (!booking_id || !collectedByName || !collectedByContact) {
      return new Response(
        "Booking and collection person details cannot be null!",
        { status: 400 }
      );
    }
    ...
    var homeCollection = new HomeCollection({
      ...
      samplesToBeCollected: Array.isArray(samples_for_pick)
        ? samples_for_pick
        : [],
      collectedBy:
        !collectedBy || collectedBy.trim() == "" ? null : collectedBy,
    ...
    await newActivity.save();
    return new Response(
      { message: "Home Collection has been created successfully!" },
      { status: 200 }
    );
  } catch (error) {
    console.log(error);
