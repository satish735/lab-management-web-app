...
export const GET = async (request) => {
  try {
    ...
    const bookingList = await Booking.find(searchFilter)
      ...
      .populate({ path: "teamMemberId", select: "name email" })
      .populate({
        path: "packages",
    ...
    // Return response
    return new Response(
      JSON.stringify({ data: bookingList, total: totalCount }),
      { status: 200 }
    );
  } catch (error) {
...
export const POST = async (request, { params }) => {
  ...
  try {
    ...
    if (
      ...
      session.endSession();
      return new Response(
        "Team member, Packages, Slot and center Id cannot be null!",
        { status: 400 }
      );
    }
    ...
    if (
      !slotCheck ||
      Number(slotCheck?.currentUse ?? 0) + team_members.length >
        slotCheck?.maxUse
    ) {
      await session.abortTransaction();
      session.endSession();
      return new Response(
        "Slots are not available for your bookings! Please try again by changing slots!",
        { status: 400 }
      );
    }
    const bookingPromises = [];
    var getLastBooking = await Booking.findOne().sort({ sequence: -1 });
    var bookingsToInsert = team_members
      .filter(
        (teamItem) =>
          teamItem?.member_id &&
          teamItem?.packages &&
          Array.isArray(teamItem?.packages) &&
          teamItem?.packages.length > 0
      )
    ...
    console.log(bookingsToInsert);
    // Booking.insertMany(bookingsToInsert, { session });

    // for (const team_member of team_members.filter(
    //   (teamItem) =>
    //     teamItem?.member_id &&
    //     teamItem?.packages &&
    //     Array.isArray(teamItem?.packages) &&
    ...
    // }
    var bookings = await Booking.insertMany(bookingsToInsert, { session });
    slotCheck.currentUse =
      Number(slotCheck?.currentUse ?? 0) + team_members.length;
